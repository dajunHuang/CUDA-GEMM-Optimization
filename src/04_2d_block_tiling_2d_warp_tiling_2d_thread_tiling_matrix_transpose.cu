...
template <typename T, size_t WARP_TILE_SIZE_X, size_t WARP_TILE_SIZE_Y,
          ...
          size_t NUM_THREAD_TILES_PER_WARP_Y, size_t NUM_THREADS_PER_WARP_X,
          size_t NUM_THREADS_PER_WARP_Y, size_t THREAD_TILE_SIZE_X,
...
#pragma unroll
                for (size_t thread_tile_y_idx{0U};
                     ...
                     ++thread_tile_y_idx)
                {
                    C[(C_row_idx + thread_tile_x_idx) +
                      (C_col_idx + thread_tile_y_idx) * ldc] =
                        alpha * C_thread_results[thread_tile_repeat_row_idx]
                                                [thread_tile_repeat_col_idx]
                                                [thread_tile_x_idx]
                                                [thread_tile_y_idx] +
                        beta * C[(C_row_idx + thread_tile_x_idx) +
                                 (C_col_idx + thread_tile_y_idx) * ldc];
                }
            }
        }
...
template <typename T, size_t BLOCK_TILE_SIZE_X, size_t BLOCK_TILE_SIZE_Y,
          ...
          size_t WARP_TILE_SIZE_Y, size_t NUM_THREADS_PER_WARP_X,
          size_t NUM_THREADS_PER_WARP_Y, size_t THREAD_TILE_SIZE_X,
          size_t THREAD_TILE_SIZE_Y>
__global__ void gemm_04(size_t m, size_t n, size_t k, T const alpha, T const* A,
                        ...
                        size_t ldc)
{
    ...
    //     (C_thread_block_tile, ldsc, warp_linear_idx,
    ...
    constexpr unsigned int NUM_LOAD_THREADS_PER_ROW_A{BLOCK_TILE_SIZE_K /
                                                      NUM_EACH_THREAD_LOAD_A};
    size_t const a_load_warp_row_idx{warp_linear_idx /
                                     NUM_LOAD_THREADS_PER_ROW_A};
    size_t const a_load_warp_col_idx{warp_linear_idx %
                                     NUM_LOAD_THREADS_PER_ROW_A};
    ...
    size_t const b_load_warp_row_idx{warp_linear_idx /
                                     NUM_LOAD_THREADS_PER_ROW_B};
    size_t const b_load_warp_col_idx{warp_linear_idx %
                                     NUM_LOAD_THREADS_PER_ROW_B};
    ...
    {
#pragma unroll
        for (size_t load_time{0}; load_time < NUM_EACH_THREAD_LOAD_A;
             ++load_time)
        {
            A_thread_block_tile[(a_load_warp_row_idx * 32U +
                                 thread_linear_idx_in_warp) +
                                (a_load_warp_col_idx * NUM_EACH_THREAD_LOAD_A +
                                 load_time) *
                                    ldsa] =
                A[(blockIdx.x * BLOCK_TILE_SIZE_X + a_load_warp_row_idx * 32U +
                   thread_linear_idx_in_warp) +
                  (thread_block_tile_idx * BLOCK_TILE_SIZE_K +
                   a_load_warp_col_idx * NUM_EACH_THREAD_LOAD_A + load_time) *
                      lda];
        }

#pragma unroll
        for (size_t load_time{0}; load_time < NUM_EACH_THREAD_LOAD_B;
             ++load_time)
        {
            B_thread_block_tile[(b_load_warp_row_idx * 32U +
                                 thread_linear_idx_in_warp) *
                                    ldsb +
                                (b_load_warp_col_idx * NUM_EACH_THREAD_LOAD_B +
                                 load_time)] =
                B[(thread_block_tile_idx * BLOCK_TILE_SIZE_K +
                   b_load_warp_row_idx * 32U + thread_linear_idx_in_warp) +
...
#pragma unroll
        for (size_t k_i{0U}; k_i < BLOCK_TILE_SIZE_K; ++k_i)
        {
            ...
            load_data_from_shared_memory_to_register_file<
                ...
                NUM_THREAD_TILES_PER_WARP_X, THREAD_TILE_SIZE_X>(
            ...
            load_data_from_shared_memory_to_register_file<
                ...
                NUM_THREAD_TILES_PER_WARP_Y, THREAD_TILE_SIZE_Y>(
            ...
            compute_thread_tile_results<
                T, WARP_TILE_SIZE_X, WARP_TILE_SIZE_Y,
                           ...
                           T const* beta, T* C, size_t ldc, cudaStream_t stream)
{
    ...
    //             WARP_TILE_SIZE_X, WARP_TILE_SIZE_Y, NUM_THREADS_PER_WARP_X,
    //             NUM_THREADS_PER_WARP_Y, THREAD_TILE_SIZE_X, THREAD_TILE_SIZE_Y>,
    ...
    gemm_04<T, BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y, BLOCK_TILE_SIZE_K,
            ...
            WARP_TILE_SIZE_Y, NUM_THREADS_PER_WARP_X, NUM_THREADS_PER_WARP_Y,
            THREAD_TILE_SIZE_X, THREAD_TILE_SIZE_Y>
...
template void launch_gemm_kernel_04<float>(size_t m, size_t n, size_t k,
                                           ...
                                           float* C, size_t ldc,
                                           cudaStream_t stream);
template void launch_gemm_kernel_04<double>(size_t m, size_t n, size_t k,
                                            double const* alpha,

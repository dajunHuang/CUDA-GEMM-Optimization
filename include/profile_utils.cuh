                     ...
                     size_t ldc)
{
    ...
    for (size_t j{0U}; j < n; ++j)
    {
        for (size_t i{0U}; i < m; ++i)
        {
            ...
            for (size_t l{0U}; l < k; ++l)
            {
                sum += __half2float(__hmul(A[i + l * lda], B[l + j * ldb]));
            }
            C[i + j * ldc] = __float2half(__half2float(*alpha) * sum +
                                          __half2float(*beta) *
                                              __half2float(C[i + j * ldc]));
        }
    }
}
...
template <typename T,
          typename std::enable_if<std::is_same<T, float>::value ||
                                      ...
                                      std::is_same<T, __half>::value,
                                  bool>::type = true>
void zero_initialize_matrix(T* A, size_t m, size_t n, size_t lda)
{
    for (size_t j{0U}; j < n; ++j)
    {
        for (size_t i{0U}; i < m; ++i)
        {
            A[i + j * lda] = static_cast<T>(0);
        }
    }
}
    ...
    unsigned int seed = 0U)
{
    ...
    CHECK_CUDA_ERROR(cudaMallocHost(&A_host, k * lda * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMallocHost(&B_host, n * ldb * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMallocHost(&C_host, n * ldc * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMallocHost(&C_host_ref, n * ldc * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMallocHost(&C_host_from_device, n * ldc * sizeof(T)));
    ...
    T* B_device{nullptr};
    T* C_device{nullptr};
    CHECK_CUDA_ERROR(cudaMalloc(&A_device, k * lda * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMalloc(&B_device, n * ldb * sizeof(T)));
    CHECK_CUDA_ERROR(cudaMalloc(&C_device, n * ldc * sizeof(T)));

    // Copy matrix A and B from host to device.
    CHECK_CUDA_ERROR(cudaMemcpy(A_device, A_host, k * lda * sizeof(T),
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(B_device, B_host, n * ldb * sizeof(T),
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(C_device, C_host, n * ldc * sizeof(T),
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(C_host_ref, C_host, n * ldc * sizeof(T),
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(C_host_ref, C_device, n * ldc * sizeof(T),
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(C_host_from_device, C_device,
                                n * ldc * sizeof(T), cudaMemcpyDeviceToHost));
